---
title: "Chill칠치 못한 Function Calling"
description: "Tool Binding, Function Calling, Tool Calling의 차이와 LLM이 실제로 API를 어떻게 호출하는지 원리를 설명한 글"
publishDate: 2025-03-15
tags: ["Research", "LLM", "Tool", "Agent"]
draft: false
---

![Chill칠치 못한 Function Calling](/ash-world/assets/function_calling_cover.png)
...
위의 정의를 살펴보면 모델이, **데이터를 가져오거나 액션을 수행하는 것을 가능하게 만드는 것**이라고 설명되어 있는데, 데이터를 가져오는 것도 특정한 개발영역의 관점에서는 GET api의 호출과 동일하므로 ****"Action을 수행하는 것"**** 하나로 정의내려볼 수 있겠다. 이 정의는 결국 모델이 필요한 시점에 필요한 요청(Request)을 날려서, API를 호출한다는 뜻이 되는데 그렇다 LLM은 어떻게 스스로 **API**를 호출한다는 걸까요?
...
* **User** : What is the weather like the Paris today?
  * _Function Calling_ : "get\_weather"
    ...
    * _Output params_ : sunny _**(예시이다 !)**_
...
![[ Fig 01. ] Re-Question Pattern](/ash-world/assets/image (2).png)
